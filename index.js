...
function getArtistByIndex(base,indexNum) {
return `the artist at index ${indexNum} is ${base[indexNum].name}`
}
...
*/
let test=[1,2,3,4,5]
function listOfNames(base) {
  let copy = base.splice(0,base.length)
...
function removeArtist(base,indexNum) {
let copy = base.splice(0,base.length)
delete copy[indexNum]
copy.splice(indexNum,1)
return copy
}
console.log(removeArtist(test,2))
...
function addArtist(base,addName,addYears,addGenre,addNation,addBio) {
  let artist = { 
    name:addName,
    years:addYears,
    genre:addGenre,
    nationality:addNation,
    bio:addBio
  }
  base.push(artist)
  return base
...
function lotsOfArt(base) {
let copy=[]
for(let i=0;i<base.length; i++){
  if(base[i].paintings>100){
    copy.push(base[i].name)
  }
}
return copy
}
...
function artistInfo(base,artistName){
  for(let i=0;i<base.length;i++){
    if(base[i].name==(artistName)){
      return base[i].bio
    }
  }
}
...
function artistByCountry(base,nation){
  let copy =[]
  for(let i=0;i<base.length;i++){
    if(base[i].nationality==(nation)){
      copy.push(base[i].name)
    }
  }
  return copy
